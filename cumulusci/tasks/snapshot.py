...
class BaseCreateOrgSnapshot(BaseDevhubTask, BaseGithubTask):
    ...
    def _run_task(self):
        ...
        set_github_output("SNAPSHOT_ID", snapshot["Id"], self.logger)
...
class CreateHashedSnapshot(BaseCreateOrgSnapshot):
    ...
    def _should_create_snapshot(self):
        ...
        if self.snapshots.existing_active_snapshot:
            self.return_values["skip_reason"] = "Existing active snapshot found"
            return False
        return True
...
class GithubPullRequestSnapshotOptions(CCIOptions):
    ...
    build_fail_tests: Optional[bool] = Field(
        default=None,
...
class GithubPullRequestSnapshot(BaseCreateOrgSnapshot):
    ...
    **Requires** *`target-dev-hub` configured globally or for the project, used as the target Dev Hub org for Scratch Org Snapshots*.
    ...
    def _run_task(self):
        if self.parsed_options.check_only:
            ...
            if should_create:
                self.console.print(
                    Panel(
                        f"Conditions met for snapshot creation",
                ...
                self.console.print(
                    Panel(
                        f"Conditions not met for snapshot creation:\n[b][red]{self.return_values.get('skip_reason','')}[/red][/b]",
                        title="Snapshot Creation Check",
                        border_style="yellow",
                    )
                )
    ...
    def _generate_snapshot_name(self, name: str = None):
        self.pull_request = self._lookup_pull_request()
        if self.pull_request:
            name = f"Pr{self.pull_request.number}"
        ...
        if self.parsed_options.build_success is False:
            if self.parsed_options.build_fail_tests:
                ...
                name = f"Fail"
        self.logger.info(f"Base snapshot name for PR: {name}")
        return super()._generate_snapshot_name(name)