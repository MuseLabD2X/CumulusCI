...
def report_diffs(diffs: List[Dict[str, Any]]) -> str:
    ...
    console = Console(width=120, file=StringIO())
    console.print(
        Text(
            "OrgConfig JSON Roundtrip Differences",
    ...
    for diff in diffs:
        ...
        base_value = (
            yaml_dump(base_value, indent=4) if base_value is not None else "N/A"
        )
        compare_value = (
            yaml_dump(compare_value, indent=4) if compare_value is not None else "N/A"
        )
...
def check_round_trip(data: dict, logger: Logger) -> Optional[bytes]:
    ...
    try:
        ...
        if diffs:
            ...
            if not report:
                logger.warning(yaml_dump(diffs))
        else:
